...
export default function Index({ achievements, from, to, auth }) {
    ...
    const filteredData = achievements?.filter((row) => {
        const shift = row.shift?.toString();
        const shiftToLower = shift ? shift.toLowerCase() : "";

        return (
            (row.date &&
                row.date.toLowerCase().includes(filterText.toLowerCase())) ||
            (shift && shiftToLower.includes(filterText.toLowerCase())) ||
            (row.user.group &&
                row.user.group
                    .toLowerCase()
            ...
            (row.remarks &&
                row.remarks.toLowerCase().includes(filterText.toLowerCase()))
        );
    });
