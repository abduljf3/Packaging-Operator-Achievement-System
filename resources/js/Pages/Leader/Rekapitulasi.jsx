...
export default function Index({ achievements, from, to, auth }) {
    ...
    const filteredData = achievements?.filter((row) => {
        const totalLot = row.totalLot?.toString();
        return (
            row.drw_no
                ?.toString()
                .toLowerCase()
                .includes(filterText.toLowerCase()) ||
            totalLot?.toLowerCase().includes(filterText.toLowerCase()) ||
            row.totalQty
                ?.toString()
                .toLowerCase()
                .includes(filterText.toLowerCase()) ||
    ...
    const pdfSubmit = (e) => {
        e.preventDefault();
        const url =
            route("leader.cetak_pdf") +
            "?" +
            new URLSearchParams(data).toString();
        window.location.href = url;
    };
    const cetak_excel_rekapitulasi = (e) => {
        e.preventDefault();
        const url =
            route("leader.cetak_excel_rekapitulasi") +
            "?" +
            new URLSearchParams(data).toString();
        window.location.href = url;
    };
    const handleImportExcel = (e) => {
        ...
        axios
            .post("/leader/import_excel", formData)
            .then((response) => {
    ...
    const columns = [
        ...
        {
            name: "Qty",
            selector: (row) => parseFloat(row.totalQty).toLocaleString("id-ID"),
            sortable: true,
        },
        {
            name: "Type",
                ...
                <div className="w-screen">
                    <div className="flex justify-between px-10 pt-2 ">
                        ...
                        >
                            ...
                            <PrimaryButton
                                type="submit"
                                className="bg-green-500 hover:bg-green-500 focus:bg-green-600 active:bg-green-600 focus:ring-lime-500"
                                text="FILTER"
                            />
                        </form>
                        {achievements && (
                            <div className="flex mr-0">
                                <div className="flex items-center gap-3">
                                    <PrimaryButton
                                        onClick={Print}
                                        type="submit"
                                        className="bg-orange-500 hover:bg-orange-500 focus:bg-orange-600 active:bg-orange-600 focus:ring-orange-400"
                                        text="PRINT"
                                    />
                                    <Dropdown>
                                        <Dropdown.Trigger>
